...
export default function DoctorHistoryPage() {
  ...
  const {
    ...
    queryKey: ["doctorAppointmentHistory"],
    queryFn: async () => {
      ...
      const response = await fetch(`${SERVER_URL}/api/dashboard_doctors_specialty/schedule/history`, {
        headers: {
    ...
    if (searchQuery) {
      result = result.filter(
        (appointment) =>
          ...
          appointment.patient_id.toLowerCase().includes(searchQuery.toLowerCase()),
      )
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="flex w-full md:w-auto gap-2">
          <div className="relative w-full md:w-64">
            ...
            <Input
              placeholder="Search by reason, ID or patient"
            ...
            <tbody>
              {filteredHistory.map((appointment) => (
                <tr key={appointment.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-4 px-4">
                    <span className="font-medium text-gray-700">{appointment.patient_id.substring(0, 10)}...</span>
                  </td>
                  <td className="py-4 px-4">
